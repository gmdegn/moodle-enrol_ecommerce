...
class enrol_elightenment_testcase extends advanced_testcase {
    ...
    */
    ...
    public function test_expired() {
        ...
        $ecomplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $ecomplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $ecomplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now - 60);

        $ecomplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $ecomplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now - 60 * 60);
        $ecomplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60 * 60);

        $ecomplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now - 60 * 60 * 24 * 7, $now - 60);
        $ecomplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $ecomplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $ecomplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));

        $ecomplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $ecomplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
